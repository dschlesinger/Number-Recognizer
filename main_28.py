...
def index():
    label = 'To testing'
    return rt('main.html', link='test', linklabel=label, title='Main')
...
def test():
    ...
    if rq.method == 'POST':
        ...
        if -60000>int(req) or 60000<int(req):
            return rt('test.html', title='Testing Web',link='index', linklabel=label, list_img=list_img, img="Out of Index",pred="Out of index no prediction",value="Not good error", version=version)
        elif int(req) > -1:
        ...
        return rt('test.html', link='index', linklabel=label, list_img=list_img,version=version, title='Testing Web')
